...
- [Provider source](https://github.com/okta/terraform-provider-okta/blob/master/okta/services/idaas/resource_okta_app_saml.go)
...
- `acs_endpoints` (List of String) An array of ACS endpoints. You can configure a maximum of 100 endpoints.
- `acs_endpoints_indices` (Block Set) ACS endpoints with indices, as a set of maps with `url` and `index` keys. (see [below for nested schema](#nestedblock--acs_endpoints_indices))
...
<a id="nestedblock--acs_endpoints_indices"></a>
### Nested Schema for `acs_endpoints_indices`
