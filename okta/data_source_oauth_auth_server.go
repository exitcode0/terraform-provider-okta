...
func (d *OAuthAuthorizationServerDataSource) Read(ctx context.Context, readReq datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var baseURL string
	if d.config.oktaSDKClientV5 != nil && d.config.oktaSDKClientV5.GetConfig().Host != "" {
		baseURL = d.config.oktaSDKClientV5.GetConfig().Host
	} else {
	...
	state.ResponseTypesSupported = convertStringSlice(metadata.ResponseTypesSupported)
	state.ResponseModesSupported = convertStringSlice(metadata.ResponseModesSupported)
	state.GrantTypesSupported = convertStringSlice(metadata.GrantTypesSupported)
	state.SubjectTypesSupported = convertStringSlice(metadata.SubjectTypesSupported)
	state.ScopesSupported = convertStringSlice(metadata.ScopesSupported)
	state.TokenEndpointAuthMethodsSupported = convertStringSlice(metadata.TokenEndpointAuthMethodsSupported)
	state.ClaimsSupported = convertStringSlice(metadata.ClaimsSupported)
	state.CodeChallengeMethodsSupported = convertStringSlice(metadata.CodeChallengeMethodsSupported)
	state.IntrospectionEndpoint = types.StringValue(metadata.IntrospectionEndpoint)
	state.IntrospectionEndpointAuthMethodsSupported = convertStringSlice(metadata.IntrospectionEndpointAuthMethodsSupported)
	state.RevocationEndpoint = types.StringValue(metadata.RevocationEndpoint)
	state.RevocationEndpointAuthMethodsSupported = convertStringSlice(metadata.RevocationEndpointAuthMethodsSupported)
	...
	state.RequestObjectSigningAlgValuesSupported = convertStringSlice(metadata.RequestObjectSigningAlgValuesSupported)
	...
	state.BackchannelTokenDeliveryModesSupported = convertStringSlice(metadata.BackchannelTokenDeliveryModesSupported)
	state.BackchannelAuthenticationRequestSigningAlgValuesSupported = convertStringSlice(metadata.BackchannelAuthenticationRequestSigningAlgValuesSupported)
	state.DpopSigningAlgValuesSupported = convertStringSlice(metadata.DpopSigningAlgValuesSupported)
